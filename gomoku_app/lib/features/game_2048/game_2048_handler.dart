import 'package:flutter/widgets.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:gomoku_app/core/game_framework/game_handler.dart';
import 'package:gomoku_app/core/utils/device_info.dart';
import 'package:gomoku_app/features/game_2048/game_2048_providers.dart';
import 'package:gomoku_app/features/game_2048/game_2048_screen.dart';
import 'package:gomoku_app/features/game_2048/game_2048_timer.dart';
import 'package:gomoku_app/models/network_message.dart';

class Game2048Handler extends GameHandler {
  final WidgetRef ref;
  final void Function(NetworkMessage) _sendMessage;
  bool _isSolo = false;
  int _myPlayerIndex = 0;

  Game2048Handler(this.ref, this._sendMessage);

  @override
  void initGame({
    required int myPlayerIndex,
    required String opponentName,
  }) {
    _isSolo = opponentName.isEmpty;
    _myPlayerIndex = myPlayerIndex;
    if (_isSolo) {
      _startSoloGame();
    } else {
      if (myPlayerIndex == 0) {
        _startPvPAsHost(opponentName);
      }
    }
  }

  Future<void> _startSoloGame() async {
    ref.read(game2048StateProvider.notifier).setLoading();
    final seed = Game2048StateNotifier.generateSeed();
    ref.read(game2048StateProvider.notifier).startGame(
      seed: seed,
      isSolo: true,
      selfName: deviceName,
    );
    ref.read(game2048TimerProvider.notifier).start();
  }

  void _startPvPAsHost(String opponentName) {
    ref.read(game2048StateProvider.notifier).setLoading();
    final seed = Game2048StateNotifier.generateSeed();
    ref.read(game2048StateProvider.notifier).startGame(
      seed: seed,
      isSolo: false,
      selfName: deviceName,
      opponentName: opponentName,
    );
    ref.read(game2048TimerProvider.notifier).start();

    _sendMessage(NetworkMessage(
      type: 'game_2048_board',
      senderId: deviceId,
      payload: {
        'seed': seed,
        'device_name': deviceName,
      },
      timestamp: DateTime.now().millisecondsSinceEpoch,
    ));
  }

  @override
  void handleMessage(NetworkMessage message) {
    switch (message.type) {
      case 'game_2048_board':
        _handleBoardMessage(message);
        break;
      case 'game_2048_won':
        _handleOpponentWon(message);
        break;
      case 'game_2048_game_over':
        _handleGameOver(message);
        break;
      case 'game_2048_rematch_request':
        _handleRematchRequest(message);
        break;
      case 'game_2048_rematch_response':
        _handleRematchResponse(message);
        break;
    }
  }

  @override
  void handleConnectionLost() {
    ref.read(game2048TimerProvider.notifier).stop();
    ref.read(game2048RematchStatusProvider.notifier).state =
        Game2048RematchStatus.none;

    final gameState = ref.read(game2048StateProvider);
    if (gameState.status == Game2048Status.playing) {
      ref.read(game2048StateProvider.notifier).handleConnectionLost();
    } else {
      ref.read(game2048AutoExitProvider.notifier).state = true;
    }
  }

  @override
  Widget buildScreen({
    required String opponentName,
    required void Function(NetworkMessage) onSendMessage,
  }) {
    return Game2048Screen(
      opponentName: opponentName,
      onSendMessage: onSendMessage,
    );
  }

  @override
  void dispose() {
    ref.read(game2048TimerProvider.notifier).stop();
    ref.read(game2048StateProvider.notifier).resetGame();
    ref.read(game2048RematchStatusProvider.notifier).state =
        Game2048RematchStatus.none;
    ref.read(game2048AutoExitProvider.notifier).state = false;
    ref.read(game2048ToastProvider.notifier).state = null;
  }

  // ========== Message Handlers ==========

  void _handleBoardMessage(NetworkMessage message) {
    if (ref.read(game2048StateProvider).status == Game2048Status.playing) {
      return;
    }

    final seed = message.payload['seed'] as int;
    final opponentName =
        message.payload['device_name'] as String? ?? '对手';

    final grid = Game2048StateNotifier.generateInitialGrid(seed);
    ref.read(game2048StateProvider.notifier).startGameWithGrid(
      grid: grid,
      seed: seed,
      isSolo: false,
      selfName: deviceName,
      opponentName: opponentName,
    );
    ref.read(game2048TimerProvider.notifier).start();
  }

  void _handleOpponentWon(NetworkMessage message) {
    ref.read(game2048StateProvider.notifier).opponentWon();
    ref.read(game2048TimerProvider.notifier).stop();
  }

  void _handleGameOver(NetworkMessage message) {
    final reason = message.payload['reason'] as String? ?? 'unknown';
    if (reason == 'quit' || reason == 'disconnect') {
      ref.read(game2048StateProvider.notifier).handleConnectionLost();
    } else if (reason == 'timeout') {
      ref.read(game2048StateProvider.notifier).opponentTimeout();
    }
    ref.read(game2048TimerProvider.notifier).stop();
  }

  // ========== Rematch Logic ==========

  void _handleRematchRequest(NetworkMessage message) {
    if (ref.read(game2048StateProvider).status == Game2048Status.playing) {
      return;
    }

    if (ref.read(game2048RematchStatusProvider) ==
        Game2048RematchStatus.waiting) {
      _sendMessage(NetworkMessage(
        type: 'game_2048_rematch_response',
        senderId: deviceId,
        payload: {'accepted': true},
        timestamp: DateTime.now().millisecondsSinceEpoch,
      ));
      _restartGame();
    } else {
      ref.read(game2048RematchStatusProvider.notifier).state =
          Game2048RematchStatus.received;
      ref.read(game2048ToastProvider.notifier).state = '对方请求了重赛';
    }
  }

  void _handleRematchResponse(NetworkMessage message) {
    final accepted = message.payload['accepted'] as bool? ?? false;
    if (accepted) {
      _restartGame();
    } else {
      ref.read(game2048RematchStatusProvider.notifier).state =
          Game2048RematchStatus.none;
      ref.read(game2048ToastProvider.notifier).state = '对方拒绝了重开请求';
    }
  }

  void _restartGame() {
    ref.read(game2048StateProvider.notifier).incrementRound();
    ref.read(game2048RematchStatusProvider.notifier).state =
        Game2048RematchStatus.none;
    ref.read(game2048TimerProvider.notifier).reset();

    if (_isSolo) {
      _startSoloGame();
    } else if (_myPlayerIndex == 0) {
      final seed = Game2048StateNotifier.generateSeed();
      final opponentName = ref.read(game2048StateProvider).opponentName;
      ref.read(game2048StateProvider.notifier).startGame(
        seed: seed,
        isSolo: false,
        selfName: deviceName,
        opponentName: opponentName,
      );
      ref.read(game2048TimerProvider.notifier).start();

      _sendMessage(NetworkMessage(
        type: 'game_2048_board',
        senderId: deviceId,
        payload: {
          'seed': seed,
          'device_name': deviceName,
        },
        timestamp: DateTime.now().millisecondsSinceEpoch,
      ));
    }
  }
}
